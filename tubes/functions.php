...
function koneksi()
{
  ...
  mysqli_select_db($conn, 'asuransi') or die("Database salah!");
  return $conn;
}
...
function hapus($id)
{
  ...
  // hapus data gambar
  if ($asuransi["gambar"] !== 'nophoto.jpg') {
    unlink('img/' . $asuransi["gambar"]);
  }

  mysqli_query($conn, "DELETE FROM asuransi WHERE id = $id") or die(mysqli_error($conn));
  return mysqli_affected_rows($conn);
}
